...
用法:
  python taiji/train/autodl_pretrain_fixed.py --size 350m --data taiji_data/training_data/pretrain_final.jsonl
"""
import os
import sys
import json
...
class TaijiDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_file, tokenizer, max_len=512):
        ...
        logger.info(f"加载完成: {count} 条样本, 跳过 {skipped} 条")
...
def train(args):
    ...
    # 查找 SentencePiece 模型
    sp_candidates = [
        "taiji/tokenizer/sentencepiece.model",
        "tokenizer_native_v2/sentencepiece.model",
        "taiji/tokenizer_native_v2/sentencepiece.model",
    ]
    sp_loaded = False
    ...
    model = ModelSelf(config).to(device)
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        logger.info(f"从 checkpoint 恢复: step={start_step}")
    ...
    while global_step < args.max_steps:
        for batch in dataloader:
            ...
            if global_step % args.eval_every == 0:
                avg_eval_loss = running_loss / args.eval_every if args.eval_every > 0 else loss_val
