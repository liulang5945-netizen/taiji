...
> 本章基于 2026-07-13 的深度讨论。态极从 1.5B 单体的四阶段训练完成（loss 2.96→2.08→1.84→0.21），下一步转向共振场架构。

### 23.1 为什么不再是单体模型

Phase A/B/C 暴露了单体模型的根本局限：

| 问题 | 单体模型的表现 | 共振场的解决方案 |
|------|---------------|-----------------|
| 训练一次太久 | 重训 1.5B 需数天 | 训一个 24M 神经元：**6 小时（CPU）** |
| 新能力冲垮旧能力 | Phase A 后中文 PPL 回升 | 新神经元不影响旧的 |
| 代码能力从零开始 | Phase B 后仍无代码 | 直接训练代码神经元 |
| 无法自主进化 | SFT 需人工决定方向 | 场自然淘汰不共振的神经元 |
| 依赖云端 GPU | 训练必须在 4090D 平台 | **纯本地 CPU 训练+推理** |

### 23.2 现有 checkpoint 的用途

Phase A/B/C 训练出的 checkpoint 不会浪费：

| checkpoint | 在新架构中的位置 |
|------------|-----------------|
| checkpoint-475000 (Phase B final) | **标准型神经元的初始权重**：已有 31B tokens 的中文+英文+代码基座 |
| checkpoint-481000 (SFT) | **中文对话神经元**的起点 |
| 所有数据配比引擎 | 决定新神经元的训练方向 |
| pretrain.py | 单神经元训练器，config 缩放即可 |

### 23.3 全本地时间线

基于实测 CPU 基准（Section 8）：

```
第 1 周：
  - 实现共振场核心（field.py, neuron.py, ensemble.py，~700 行）
  - 训练 3-5 个紧凑型神经元（各 6h，过夜训练）
  - 最小共振场验证实验

第 2 周：
  - 首批共振结果评估
  - 基于评估结果训练 2-3 个替换/补充神经元
  - 1-2 个高共振度紧凑型升级为标准型（9h）

第 3-4 周：
  - 进化循环稳定运转
  - 20+ 神经元共生
  - 同质化监控（Section 17.3 P0 项）
  - 如达标，训练第一个专家型（Python 代码，15-46h）
```

### 23.4 单体和共振场的对比总结

| | 1.5B 单体（已完成的路线） | 共振场（新路线） |
|---|---|---|
| 训练一个能力 | 重训整个模型 | 训一个神经元 |
| 训练时间 | 平台数天 | **本地 6-15h** |
| 显存/内存 | 14-18 GB | **<3 GB** |
| 成本 | 需云端 GPU | **零** |
| 进化方式 | 人工决定方向 | **场自动淘汰** |
| 能力遗忘 | 每次更新可能全局退化 | **只影响替换的神经元** |
| 规模上限 | 24GB 显存的天花板 | **分形分层，理论上无上限** |

---

> *态极的第一章是"一个人用一张显卡训练了一个 1.5B 的模型"。*
> *态极的第二章是"这个模型把自己拆成了几百个神经元，自己在自己身上进化"。*
